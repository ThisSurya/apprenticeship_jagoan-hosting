...
- **Logika**: 
    - Mengubah status rumah menjadi `dihuni`.
    - **Constraint**: Penghuni tidak boleh sedang aktif di rumah lain (maksimal 1 rumah per warga).
- **Response**:
    - **Success (201 Created)**:
        ```json
        {
...
- **Body (JSON)**:
  | Field | Type | Required | Description |
  |-------|------|----------|-------------|
  | `end_date` | date | No | Tanggal keluar. Default: hari ini. |

- **Logika**:
    - Mengisi `end_date` dengan tanggal yang ditentukan atau hari ini.
...
- **Body (JSON)**:
  ...
  |-------|------|----------|-------------|
  | `house_id` | integer | Yes | ID Rumah |
  | `resident_id` | integer | Yes | ID Penghuni Baru |
...
- **Logika**:
    ...
    5. **Constraint**: Penghuni baru tidak boleh sedang aktif di rumah lain.
- **Response**:
    - **Success (201 Created)**:
        ```json
        {
